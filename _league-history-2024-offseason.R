...
get_schedule_history_sleeper <- function(season,
                                          league_id,
                                          platform) {
  ...
  df <- ff_schedule(local_connection) %>% 
    left_join(ff_franchises(local_connection) %>%
                select(franchise_id, user_name), by = "franchise_id")
...
espn_schedule_history <- 
  tibble(season = c(2015:2020)) %>%
  mutate(nested_records = map(season, get_schedule_history_espn)) %>% 
  unnest(cols = nested_records)


sleeper_schedule_history <- league_id_history %>% 
  mutate(yearly_standings = pmap(.l = list(season = season,
                                           league_id = league_id,
                                           platform = platform),
                                 .f = get_schedule_history_sleeper))%>% 
  unnest(cols = yearly_standings)
