...
scoring_histoy <- ff_scoringhistory(conn, season = 2015:2020) %>%
  filter(week <= 16, pos %in% c("QB","RB","WR","TE")) %>% 
  group_by(season, espn_id, pos, gsis_id) %>% 
  ...
  select(season, espn_id, ppg_rank, ppg_rank_value, games, games_played_percent)
...
draft_history_join <- 
  ...
  left_join(scoring_histoy, by = c("season", "player_id" = "espn_id")) %>%
